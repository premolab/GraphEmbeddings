...
class OptLossGraph2vec(Graph2vec):
    ...
    def dist_mtx(self, emb_minibatch):
        # l2 distance
        translation_vectors = emb_minibatch.reshape((emb_minibatch.shape[0], 1, -1)) - emb_minibatch.reshape(
            (1, emb_minibatch.shape[0], -1))
        sim = (translation_vectors ** 2).sum(2)
        return sim

    def get_loss(self, sim_pos, sim_neg):
        part_pos = self.pos_scale * T.sum((sim_pos - 1) ** 2)
        part_neg = self.neg_scale * T.sum((sim_neg - 10) ** 2) + T.sum(-T.log(1e-20 + sim_neg))
